...
public class SPCDataDAO extends DAO<SPCData> {
     ...
     * 
     * @param measurement
     *            SPCMeasurement related to the SPCData being saved. Contains
     *            reference to the database table in which to save the SPCData
     * @param entities
     *            List of SPCData to be saved
     * @throws DAOException
     *             if exception is thrown by the database
     * @throws SQLException
     */
    public void addEntities(SPCMeasurement measurement, List<SPCData> entities) throws DAOException {
        ...
        try {
            ...
            for (SPCData entity : entities) {
                statement = connection.prepareStatement("insert into " + measurement.getTableId()
                                + " values(default,?,?,?,?,?);");
            ...
            LOG.error("SPCDataDAO:SQLException raised when adding List of Entities to database:"
                            + se.getMessage());
            try {
                connection.rollback();
                ...
                LOG.error("SPCDataDAO:SQLException raised when closing connection to database:"
                                + se.getMessage());
