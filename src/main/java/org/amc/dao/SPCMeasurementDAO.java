...
public class SPCMeasurementDAO extends DAO<SPCMeasurement> {
    ...
    public void addEntity(SPCMeasurement entity) throws DAOException {
        ...
        String tableName = "";
        /*
         * The SPCMeasurement has no tableId it must be new so create a new
         * table and save the table's name in the tableId column
         */
        if (entity.getTableId() == null || entity.getTableId().trim().equals("")) {
            tableName = getUniqueTableName(entity);
            queryString = "create table " + tableName + " (id int NOT NULL AUTO_INCREMENT, "
                            + "date Date NOT NULL," + "user_id int NOT NULL,"
                            + "measurementNumber int NOT NULL," + "measurement_id int NOT NULL,"
                            + "measurement float NOT NULL, " + "PRIMARY KEY(id))  ENGINE=InnoDB";
        ...
        try {
            ...
            LOG.error("DAO<" + getEntityClass().getSimpleName()
                            + ">:Error has occurred when trying to add entity");
        ...
        try {
            if (entity.getTableId() != null && !entity.getTableId().trim().equals("")) {
                ...
                if (doesTableExists(entity.getTableId())) {
                    ...
                    em.getTransaction().rollback();// todo Check to see if this
                                                   // works
                    em.close();
