...
public class SPCDataFormValidator implements Validator {
    ...
    public void validate(Object obj, Errors errors) {
        ...
        if (form.getMeasurement() == null) {
            errors.rejectValue("measurement", "SPCDataForm.measurement.format",
                            "Measurements not numbers ");
        } else {
            if (form.getMeasurement().contains(null)) {
                errors.rejectValue("measurement", "SPCDataForm.measurement.missing",
                                "Missing values");
            }
        }
        if (form.getMeasurementNumber() == null) {
            errors.rejectValue("measurementNumber", "SPCDataForm.measurementNumber.format",
                            "Neasurement numbers(n) not integers");
        } else {
            if (form.getMeasurementNumber().contains(null)) {
                errors.rejectValue("measurementNumber", "SPCDataForm.measurementNumber.missing",
                                "Missing measurement numbers(n)");
            }
        }

        SPCMeasurement spcMeasurement = form.getSpcMeasurement();
        if (spcMeasurement == null) {
            errors.rejectValue("spcMeasurement", "SPCDataForm.spcmeasurement.missing",
                            "Missing SPCMeasurement object");
        } else {
            int requiredSize = spcMeasurement.getNoOfMeasurements();

            if (form.getMeasurement() != null && form.getMeasurement().size() != requiredSize) {
                errors.rejectValue("measurement", "SPCDataForm.measurement.size",
                                "Too few/Many values");
            }
            if (form.getMeasurementNumber() != null
                            && form.getMeasurementNumber().size() != requiredSize) {
                errors.rejectValue("measurementNumber", "SPCDataForm.measurementNumber.size",
                                "Too few/Many values");
            }
        }
