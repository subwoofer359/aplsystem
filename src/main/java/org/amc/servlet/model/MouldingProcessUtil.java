...
public final class MouldingProcessUtil {
    ...
    public static float getTotalInjectionTime(MouldingProcess process) {
        ...
        float[] position = { process.getShotSize() + process.getPosTran(),
                process.getInjSpeedPosition_1(), process.getInjSpeedPosition_2(),
                process.getInjSpeedPosition_3(), process.getInjSpeedPosition_4(),
                process.getInjSpeedPosition_5(), process.getInjSpeedPosition_6() };

        float[] speed = { process.getInjectionSpeed_1(), process.getInjectionSpeed_2(),
                process.getInjectionSpeed_3(), process.getInjectionSpeed_4(),
                process.getInjectionSpeed_5(), process.getInjectionSpeed_6() };
    ...
    private static float getDistance(float intialVelocity, float timeInSeconds, float acceleration) {
        return (float) ((intialVelocity * timeInSeconds) + (0.5 * acceleration * Math.pow(
                        timeInSeconds, 2)));
     ...
     * @return acceleration
     */
    private static float getAcceleration(float intialVelocity, float finalVelocity,
                    float timeInSeconds) {
    ...
    public static float[][] getOldMouldClosingTimeData(MouldingProcess process) {
        ...
        float expectedDistance1 = process.getMouldClosingOpenLimitPos()
                        - process.getMouldClosedLimitPos();
        ...
        data[5][1] = distanceF;
        data[5][2] = speed3;
        // After reaching the mould protect point(ClsSPPos) the mould is
        // accelerated to when the mould faces meet
    ...
    /**
     * todo Fix the problem when acceleration/deceleration distance is greater
    ...
    public static float[][] getMouldClosingTimeData(MouldingProcess process) {
        float[][] data = new float[8][3];
        /*
         * Calculate the time taken from mould protect position to closed
         * position Initial velocity is zero Distance: Mould protect position to
         * zero = Mould protect position t=sqrt(2*s/a)
         */
        float closingTime = (float) Math.sqrt((2 * process.getClsSPPos())
                        / CLAMP_CLOSING_ACCELERATION);
        ...
        /*
         * Calculate the time taken to reach mould protect position where mould
         * velocity is zero Initial velocity is CLS SP Speed Final velocity is
         * zero (At mould protect position) Distance: Time taken to decelerate
         * to zero velocity. Time/Distance is inclusive of the time at mould cls
         * sp pos
         */
        data[data.length - 2][0] = getTime(process.getClsSPSpeed(), 0, CLAMP_CLOSING_DEACCELERATION);
        data[data.length - 2][1] = getDistance(process.getClsSPSpeed(), data[data.length - 2][0],
                        CLAMP_CLOSING_DEACCELERATION);
        ...
        float[] speedData = { process.getMouldClosingOpenLimitSpeed(),
                process.getMouldClosedLimitSpeed(), process.getClsSPSpeed() };
        ...
        while (i < posData.length) {
            ...
            i++;
            t = t + 2;// Store result in every odd index in the result array so
                      // we can calculate the even index results later

        }
        // Reset Counters and temporary variables
        i = 0;
        t = 1;// T=1 so we can work on the even index results in the results
              // array (data[][])
        distance = 0;
        speed = 0;
        time = 0;
        while (i < posData.length) {
            ...
            /*
             * The initial mould acceleration distance is included in the
             * distance ClosingOpenLimitPos->ClosedLimitPos Between the initial
             * acceleration and final deceleration the time/distance is halved
             * between each set distance
             */
            if (i == 0) {
            ...
            // If the distance change is greater than the distance of pre set
            // position then distance is set to zero
